...
export default function UserItem({ user, onEdit, isBeingEdited, isCurrentUser }: UserItemProps) {
        ...
        <div className="flex items-center animate-fade-in-down justify-between dark:bg-[#252525] bg-gray-200 p-4 rounded-2xl w-full border dark:border-[#1a1a1a] border-gray-300 shadow-sm shrink-0">
            ...
            <div>
                <button
                    ...
                    style={!isBeingEdited ? { viewTransitionName: `user-edit-${user.id}` } as React.CSSProperties : undefined}
                    className={`flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-full transition-all duration-300 ${isCurrentUser
                        ? 'dark:bg-[#1a1a1a] bg-gray-300 text-gray-400 dark:text-gray-600 cursor-not-allowed opacity-50'
                        : 'dark:bg-[#252525] text-black dark:text-gray-200 hover:dark:bg-[#111] hover:bg-gray-100 cursor-pointer hover:dark:text-white'
                        }`}
                >
