...
export default function SedeUserContent({
  users,
  usersNotInSede,
  roles,
  idSede,
  sessionUserId,
}: SedeUserContentProps) {
  ...
  async function confirmDelete() {
    ...
    if (res.success) {
      console.log(
        `The user ${userToDelete.nombre} has been successfully deleted.`,
      );
      router.refresh();
        ...
        <div className="flex flex-col gap-4 relative">
          ...
          <AnimatePresence>
            {showEditModal && (
              <SedeEditUserModal
                userProps={{
                  user: editingUser as unknown as UserProps["user"],
                }}
                roles={roles}
