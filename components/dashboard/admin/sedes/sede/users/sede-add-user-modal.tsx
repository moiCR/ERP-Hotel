...
interface SedeAddUserModalProps {
    onClose?: () => void;
    usersNotInSede?: (Usuario & { rol?: { id: number; nombre: string }, sede?: { id: number; ciudad: string } })[];
    id: number;
}

export default function SedeAddUserModal({ onClose, usersNotInSede, id }: SedeAddUserModalProps) {
                ...
                <div className="flex flex-col gap-2 bg-gray-200 dark:bg-black/10 w-[1200px] h-[500px] overflow-y-auto p-4 rounded-lg">
                    {usersNotInSede && usersNotInSede.length > 0 ? (
                        ...
                        <div className="flex items-center justify-center h-full">
                            <p className="text-gray-500 dark:text-gray-400">No hay usuarios disponibles para agregar</p>
                        </div>
                    )}
                </div>
            </ModalBody>
...
interface SedeUserNotInSedeItemProps {
    user: Usuario & { rol?: { id: number; nombre: string }, sede?: { id: number; ciudad: string } };
    isSelected?: boolean;
    onClick?: () => void;
}

function SedeUserNotInSedeItem({ user, isSelected, onClick }: SedeUserNotInSedeItemProps) {
    return (
        <div
            className={cn(
                ...
                isSelected && "bg-green-50 dark:bg-green-600/10 border-green-500 dark:border-green-500"
            )}
            onClick={onClick}
        >
            <div className="flex flex-col gap-1 flex-1">
                <p className={cn(
                    "text-sm font-medium",
                    isSelected ? "text-green-700 dark:text-green-300" : "text-gray-900 dark:text-gray-100"
                )}>
